...
|-------------------------------|---------------------|--------------------------------|
| `--cmd='/usr/bin/php -v'`     | not required        | command for run on             |
| `--comment='My test command'` | null                | just comment for list command  |
| `--id=123321`                 | generated by script | command id. optional           |
| `--delay=0`                   | 0                   | delay before run in seconds    |
 | `--tries=1`                   | 1                   | tries for run before kill job  |
 | `--tries_delay=180`           | 180 (3 min)         | delay between tries in seconds |
 
## "worker" command parameters:
| Parameter       | Default | Description                                                                          |
|-----------------|---------|--------------------------------------------------------------------------------------|
| `--single-mode` | `false` | true or false. Prevent second instance from starting                                 |
 | `--silent-mode` | `false` | true or false. All output send to log file.                                          |
